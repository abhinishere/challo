...
import 'package:challo/models/video_info_model.dart';
...
//import 'package:challo/widgets/videoplayerwidget.dart';
...
class _VideoPlayerPageState extends State<VideoPlayerPage> {
  ...
  late String onlineuid, onlinename, onlinepic, onlineusername;
  ...
  void initState() {
    ...
    getOnlineData().then((_) => {
          getStreamData().then((_) => {
                loadStreamConfig().then((_) => {
                      initializeStream(),
                    }),
              }),
        });
  ...
  Future getStreamData() async {
    ...
    formattype = videodoc['type'];

    List<String> blockedbylist = List.from(videodoc['blockedby']);

    participateduids = List.from(videodoc['participateduids']);
    ...
    QuerySnapshot usersdoc =
        await contentcollection.doc(widget.docName).collection('users').get();

    userslist = usersdoc.docs;

    print("Userslist length is ${userslist.length}");

    if (userslist.length == 1) {
      uidList = [userslist[0]['uid']];
      UserInfoModel user0 = UserInfoModel(
        uid: userslist[0]['uid'],
      ...
      videoinfo = VideoInfoModel(
        subject: videodoc['topic'],
        description: videodoc['description'],
        category: videodoc['category'],
        formattype: videodoc['type'],
        videourl: videodoc['link'],
        guestsno: videodoc['guestsno'],
        liveviews: videodoc['liveviews'].length,
        totalviews: videodoc['totalviews'].length,
        likes: videodoc['likes'].length,
        dislikes: videodoc['dislikes'].length,
        user0: user0,
        time: videodoc['time'],
      );
    }
    ...
    if (userslist.length == 2) {
      uidList = [userslist[0]['uid'], userslist[1]['uid']];
      if (formattype == 'Debate') {
        UserInfoModel user0 = UserInfoModel(
            ...
            name: userslist[0]['name'],
            pic: userslist[0]['pic'],
            selectedRadioStand: userslist[0]['stand']);
        UserInfoModel user1 = UserInfoModel(
          uid: userslist[1]['uid'],
          username: userslist[1]['username'],
          name: userslist[1]['name'],
          pic: userslist[1]['pic'],
          selectedRadioStand: userslist[1]['stand'],
        );
        videoinfo = VideoInfoModel(
          subject: videodoc['topic'],
          description: videodoc['description'],
      ...
      uidList = [userslist[0]['uid'], userslist[1]['uid'], userslist[2]['uid']];
      if (formattype == 'Debate') {
        UserInfoModel user0 = UserInfoModel(
          uid: userslist[0]['uid'],
          username: userslist[0]['username'],
        ...
        UserInfoModel user2 = UserInfoModel(
          ...
          pic: userslist[2]['pic'],
          selectedRadioStand: userslist[2]['stand'],
        );
        videoinfo = VideoInfoModel(
          subject: videodoc['topic'],
          description: videodoc['description'],
  ...
  Future initializeStream() async {
    ...
    setState(() {
      print("video url is ${videoinfo.videourl}");
  ...
  Widget moreOptions() {
    return Padding(
      ...
      child: InkWell(
        onTap: () {
          if (participateduids.contains(onlineuid)) {
            showMoreOptionsPopUpForOP();
          } else {
            showMoreOptionsPopUpForViewers();
          }
  ...
  Widget vidtypeTextWidget() {
    return Text(
      (formattype == 'QnA') ? "Q&A" : formattype,
      style: Theme.of(context)
          .textTheme
          .bodyText1!
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          InkWell(
            onTap: () => ShareService.shareContent(widget.docName, 'streampost',
                videoinfo.subject!, videoinfo.description!, ''),
            child: Padding(
              padding: const EdgeInsets.all(5.0),
              child: Column(
                mainAxisSize: MainAxisSize.min,
      ...
      child: InkWell(
        ...
        child: Column(
          ...
          children: [
            Align(
              child: Text(
                truncateWithEllipsis(27, videoinfo.subject!),
                style: styleTitleSmall(),
              ),
            ),
            const SizedBox(height: 8.0),
            Text(
              (videoinfo.totalviews == 0)
                  ? truncateWithEllipsis(30,
                      "${videoinfo.totalviews} views • ${tago.format(videoinfo.time.toDate())}")
                  : (videoinfo.totalviews == 1)
                      ? truncateWithEllipsis(30,
                          "${videoinfo.totalviews} view • ${tago.format(videoinfo.time.toDate())}")
                      : truncateWithEllipsis(30,
                          "${videoinfo.totalviews} views • ${tago.format(videoinfo.time.toDate())}"),
              style: Theme.of(context).textTheme.subtitle1!.copyWith(
                    fontSize: 13.0,
  ...
  Widget buildVideo() {
    return InkWell(
      ...
      onTap: () {
        print("screen tapped; now show/hide controls");
  ...
  Widget build(BuildContext context) {
    ...
    return (videoinforeceived == true &&
        ...
        ? (Platform.isIOS)
                            ...
                            onTap: () {
                              if (widget.hideNavLinkReturn != null) {
                                ...
                                if (showcomments == true ||
                                    showdescription == true) {
                                  setState(() {
                                    //showcomments = false; //removed for iOS
                                    //showdescription = false; //removed for iOS
                                    whetherexited = true; //added for iOS
                                    hidenav = false; //added for iOS
                                  });
                                  AppBuilder.of(context)!
                                      .rebuild(); //added this for iOS
                                  Navigator.of(context)
                                      .pop(); //added this for iOS
                                } else {
                                  setState(() {
                                    whetherexited = true;
                                    hidenav = false;
            ...
            : WillPopScope(
                onWillPop: () {
                  //for Android when comments/description widget open, back button closes them instead of popping to prev page
                  if (widget.hideNavLinkReturn != null) {
                    setState(() {
                      whetherexited = true;
                    });
