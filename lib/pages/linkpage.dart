...
class _LinkPageState extends State<LinkPage> {
  ...
  String? fullUrl,
      ...
      linkDescription,
      linkImage;
  late List<String> linkLikes, linkDislikes, linkViews;
  ...
  getlinkdata() async {
    ...
    var linkdoc = await contentcollection.doc(widget.docName).get();

    List<String> blockedbylist = List.from(linkdoc['blockedby']);
    ...
    fullUrl = linkdoc['link'];
    linkTopic = linkdoc['topic'];
    linkDescription = linkdoc['description'];
    linkImage = linkdoc['image'];
    linkDomain = linkdoc['domainname'];
    linkLikes = List.from(linkdoc['likes']); //likes
    linkDislikes = List.from(linkdoc['dislikes']); //dislikes
    linkViews = List.from(linkdoc['totalviews']); //totalviews
    timeofposting = linkdoc['time'];

    whethercommunitypost = linkdoc['whethercommunitypost'];
    communityName = linkdoc['communityName'];
    communitypic = linkdoc['communitypic'];

    opuid = linkdoc['opuid'];
    opusername = linkdoc['opusername'];
    oppic = linkdoc['oppic'];

    totallikes = linkLikes.length;
    totaldislikes = linkDislikes.length;

    if (linkLikes.contains(onlineuid)) {
      setState(() {
        likedpost = true;
      });
    } else {
    ...
    if (linkDislikes.contains(onlineuid)) {
      setState(() {
        dislikedpost = true;
      });
    } else {
    ...
    totalcomments = linkdoc['commentcount'];
              ...
              child: InkWell(
                ...
                child: Container(
                    ...
                    child: Padding(
                        padding: const EdgeInsets.only(
                            bottom:
                                10.0), //some spacing to the child from bottom
                        child: Container(
                          height: 40,
                              ...
                              children: [
                                ...
                                const Padding(
                                  ...
                                  child: Icon(Icons.open_in_browser,
                                      color: kIconSecondaryColorDark, size: 20),
                                ),
                              ],
                            ),
                          ),
          ...
          Padding(
            ...
            child: UpDownVoteWidget(
                ...
                onShared: () => ShareService.shareContent(widget.docName,
                    'linkpost', linkTopic!, linkDescription!, linkImage!),
